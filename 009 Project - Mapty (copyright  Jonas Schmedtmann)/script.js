...
class App {
  ...
  _showForm(mapE) {
    ...
    this.#lastMarker
      ?.getElement()
      .parentNode.removeChild(this.#lastMarker?.getElement());
  ...
  _newWorkout(e) {
    ...
    if (type === "running") {
      ...
      if (!checkNum(cadence, distance, duration)) {
        alert("Input has to be positive numbers");
      }
    ...
    if (type === "cycling") {
      ...
      if (!checkNum(distance, duration) || !Number.isFinite(elevation)) {
        alert("Input has to be positive numbers");
      }
